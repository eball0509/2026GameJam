...
namespace EasyPrimitiveAnimals
{
    public class AnimalController : MonoBehaviour
    {
        ...
        private void Start()
        {
            // Changed from CompareTag to string comparison
        ...
        {
            // Changed from CompareTag to string comparison
            if (this.gameObject.tag != "Chicken") // If animal the script is attached to is not a chicken, animate 4 legs and body.
            {
        ...
        {
            // Changed from CompareTag to string comparison to prevent the "Tag not defined" error
            if (collision.gameObject.tag != "Ground" && canRotate) // If the animal collides with something that is not the ground, spin it around.
            {
