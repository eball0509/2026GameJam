...
public class GameplayUIManager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        // Run the automatic marker generator cleanly at runtime start
        GenerateMarkers();
    }
    ...
    // FIX: Using Unity's built-in platform conditional to prevent crashes during scene shifts
#if UNITY_EDITOR
    private void OnValidate()
    {
        // We defer the generation using Unity's editor application delay loop 
        // to completely avoid the Awake/CheckConsistency/OnValidate instantiation crash!
        UnityEditor.EditorApplication.delayCall += () =>
        {
            // Safeguard against missing references if the object was destroyed mid-call
    ...
    private void GenerateMarkers()
    {
        ...
        // Prevent duplicate generation loops if we are already playing
        if (Application.isPlaying && markerParent.childCount > 1)
        {
            // If already generated by OnValidate right before hitting play, skip doing it again
            bool hasTicks = false;
